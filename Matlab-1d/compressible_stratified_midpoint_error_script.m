...
for i = 1:Nx
    U_exact(i) = exp(-0.5 * N_sq *X(i)) * sin(m*X(i))*sin(sigma*(t+0.125)); % Velocity
    U_exact(Nx+i) = exp(-0.5 * N_sq *X(i)) * (N_sq / (2 * sigma)* sin(m*X(i)) + m / sigma * cos(m *X(i)) )*cos(sigma*(t+0.125)); % Density
